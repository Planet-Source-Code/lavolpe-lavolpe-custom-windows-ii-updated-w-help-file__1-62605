...
' this sample form will get whatever characteristics are set for frmTest
' This is done via a PropertyBag created by the DLL's ExportCustomSettings method

' Why would you want to use that method? I can think of 2 immediate reasons
' 1. You want to provide a user with a selection of "skins" for your window but do not
'       want to give the user control over individual settings. You can create your
'       "skins" and save them via the ExportCustomSettings during design and save
'       the them in a resource file or as files (your choice). Then a user can select
'       the peferred skin from a menu or other options & you can apply them simply by
'       calling the DLL's ImportCustomSettings method.
' 2. The same basic reason as #1, except you want to give the user ability to modify
'       the window partially or completely. Then you could save the changes into the
'       user's temp folder to be recalled when the window starts up

' Both of the scenarios above are assuming you are creating the skins or modifications
' for the same application. Therefore, your basic code in the Implementation Intefaces
' won't change.

' Another side note. You will notice I used an ImageList on frmTest to hold the menu
' item images. There is no ImageList control on this form. That is because this form
' is using the propertybag (byte array file) created by the ExportCustomSettings
' function. The needed images are in that file as stdPicture objects. Therefore, we
' will use an array of stdPicture objects for the menu images. Also note that frmTest
' had Image Controls for the custom border corners. Same rule here, we don't need the
' Image Controls, and are using a stdPicture array instead.

' Now the above being said, you don't have to use arrays vs the controls. I could have
' easily added 2 Image Controls and assigned the images from the propertybag to those
' controls. I could have also added the menu item images to an ImageList control if
' I wanted to. The slight advantage of using a stdPicture array is that we do not
' need to create a dependency on the large OCX containing the ImageList.

' To use a RES file, you could save the custom settings when you are completely satisified
' that all settings for both the active/inactive window are finished. The settings can
' be saved basically by you running the application and clicking a button to save the
' setting, then remove or hide/disable the button. Now you can load the settings into
' your RES file (as a custom item) before you compile your project. You can add as many
' of these settings as you wish.

' The example in Form_Load shows importing the custom settings from a file on the
' hard drive, but to use a RES file to import, run similar code to the following....

'    lvCW.ImportCustomSettings LoadResData(101, "Custom"), Me, Me, [myPBag]

'   ^^ supply correct index vs 101 & supply optional PropertyBag parameter if wanted
...
Private menuImages() As StdPicture
Private borderImages() As StdPicture
Private polyPts(0 To 8) As POINTAPI     ' create custom menu hover border

' the saved settings do not include any values that the following interfaces would
' be referencing: iControlsCallback or iCoreMessages
Implements iOwnerDrawn                  ' allows owner drawing non client
Implements iSysTrayCallback             ' allows callbacks for the system tray
Implements iCoreMessages                ' allows subclassing the subclasser
                                        ' ^^ used to test for custom system menu click


Private Sub Form_Load()
    
    Dim myPBag As PropertyBag
    ' note: may not be compatible with PropertyBag_VB5
    ' I don't have VB5 & would need someone to help test/troubleshoot
    
    Dim I As Integer, nrImages As Integer, bRtn As Boolean
    
    mnuMain(1).Enabled = False
    
    Set lvCW = New CustomWindow     ' create instance of the DLL
    ' start subclassing, you cannot apply any saved settings until the subclassing starts
    lvCW.BeginCustomize Me.hwnd
    
    bRtn = lvCW.ImportCustomSettings(App.Path & "\_TestClone.pbg", Me, Me, myPBag)
    
    ' Example if the custom settings were saved in a Resource (.RES) file
'    bRtn = lvCW.ImportCustomSettings(LoadResData(101, "Custom"), Me, Me, myPBag)
        
            
        
    If bRtn = False Then
        ' something is wrong here, I supplied a property bag with menu images
        ' where did it go?  Corrupted file maybe?  DLL error?
        ' ^^ Good news-- haven't come across this scenario yet w/o forcing it
        Set lvCW = Nothing
        
    Else
    
        nrImages = myPBag.ReadProperty("NrImages", 0)
        
        If nrImages = 0 Then ' something is wrong here cause I saved the images
            ' ^^ Good news-- haven't come across this scenario yet
            Set lvCW = Nothing
            
        Else
            ' let's load our menu & button images
            ReDim menuImages(1 To nrImages)
            For I = 1 To nrImages
                Set menuImages(I) = myPBag.ReadProperty("Img" & I, Nothing)
            Next
            ' let's load the custom border corners
            ReDim borderImages(0 To 1)
            For I = 0 To 1
                Set borderImages(I) = myPBag.ReadProperty("Brdr" & I, Nothing)
            Next
            
            ' add the system menu add on
    ...
    Text1.Text = "Clone is probably not accurate. What you are doing is simply exporting all the styles and properties " & _
        "of the custom window from one form and importing/applying it to another form, this form." & vbCrLf & vbCrLf & _
        "The export can be used as a file or uploaded into a resource file. The DLL will import that information in either format." & vbCrLf & vbCrLf & _
        "Also, the export can contain a property bag of your choosing where you can add other information to the export, " & _
        "such as images, fonts, cursors, strings, etc.  This is a handy way of sharing custom window settings, allowing users " & _
        "to select a range of custom settings you pre-processed, or even allowing users to change some settings and then " & _
        "saving those settings to apply to their window each time your application runs." & vbCrLf & vbCrLf & _
        "As you play with the samples, anytime you want, click on this example again."
...
If IsArrayEmpty(Not menuImages) Then Exit Function
...
Dim clipRgn As Long
    ...
    If CID.itemData > 0 Then    ' then we assigned an icon to this menu item
        ...
        ' when window is inactive (wParam=0) or the menu item is disabled,
        ' draw then icon with different shading vs the blah gray
        If (CID.itemState And mcDisabled) = mcDisabled Or wParam = 0 Then
        
            lvCW.Graphics.DrawImageIcon menuImages(CID.itemData).Handle, _
...
'    If wParam = 1 Then  ' active window
        ...
        With BKG.rcItem
            ...
            lvCW.Graphics.ImageFillEx BKG.hdc, .Left, .Top, .Right, .Bottom, menuImages(13), bsTiled, , (wParam = 0)
...
Dim clipRgn As Long
    ...
    With BKG
        ...
        If wParam = 1 Then ' active window
            lvCW.Graphics.GradientFillEx .hdc, .rcItem.Left + 3, .rcItem.Top + 3, .rcItem.Right - 3, .rcItem.Bottom - 3, False, &H24CACE, &H80FFFF
        Else
            lvCW.Graphics.GradientFillEx .hdc, .rcItem.Left + 3, .rcItem.Top + 3, .rcItem.Right - 3, .rcItem.Bottom - 3, False, RGB(162, 162, 162), RGB(211, 211, 211)
        End If
    ...
    If Not IsArrayEmpty(Not borderImages) Then
        ' don't modify the following one bit! Using .Render & if used wrong: crash
        With borderImages(0)
    ...
    With CID
        ...
        Case 0
            ...
            If (.itemState And mcHover) = mcHover Then
                lvCW.Graphics.DrawImageIcon menuImages(12).Handle, .hdc, .rcItem.Left - 1, .rcItem.Top
            Else
                lvCW.Graphics.DrawImageIcon menuImages(11).Handle, .hdc, .rcItem.Left - 1, .rcItem.Top
            End If
